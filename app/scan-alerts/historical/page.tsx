...
function ScanAlertsHistoricalInner() {
  ...
  useEffect(() => {
    const fetchMonths = async () => {
      try {
        const response = await fetch('/api/scan-alerts/historical?metadata=true');
  ...
  const years = useMemo(() => {
    const set = new Set(months.map(m => m.yearMonth.slice(0, 4)));
    return [...set].sort((a, b) => b.localeCompare(a));
  }, [months]);

  const monthsForYear = useMemo(() => months.filter(m => m.yearMonth.startsWith(year)), [months, year]);

  // When the selected month changes, fetch trade/expiry dates scoped to it.
  useEffect(() => {
    if (!month) return;
    setTradeDate('');
    setExpiryDate('');
    const fetchScopedMeta = async () => {
      try {
        const response = await fetch(`/api/scan-alerts/historical?metadata=true&month=${month}`);
        if (!response.ok) throw new Error('Failed to fetch date options');
        const result = await response.json();
        if (result.success) {
        ...
        console.error('Error fetching scoped metadata:', err);
      }
    };
    fetchScopedMeta();
  }, [month]);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        setError(err instanceof Error ? err.message : 'Failed to load historical alerts');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  if (monthsLoading) return <><Header /><LoadingSpinner message="Loading historical scan alerts..." /></>;
    ...
    return (
      <>
        ...
        <div className="min-h-screen bg-gray-50 flex items-center justify-center">
          <p className="text-gray-400 text-sm">No historical scan alert data available.</p>
        </div>
      </>
    );
  }
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={e => {
                        ...
                        setYear(y);
                        const first = months.find(m => m.yearMonth.startsWith(y));
                        if (first) setMonth(first.yearMonth);
                      }}
                    ...
                    <select value={month} onChange={e => setMonth(e.target.value)} className={selectClass}>
                      {monthsForYear.map(m => (
                        <option key={m.yearMonth} value={m.yearMonth}>
                          {MONTH_NAMES[parseInt(m.yearMonth.slice(5, 7), 10) - 1]} ({m.count})
                        </option>
                      ))}
                    </select>
                  </div>
          ...
          {isLoading ? (
            ...
            <>
              ...
              <QuadrantChart
                ...
                height={560}
                title="Historical Scan Alerts Ladder"
...
export default function ScanAlertsHistoricalPage() {
  return (
    <Suspense fallback={null}>
      <ScanAlertsHistoricalInner />
    </Suspense>
  );
}
